...
class action_prediction:
	...
	def __init__(self, feature_vector, action_vector, learning_rate, name='action_predicter'):
	...
	def sequence_minibatch(self, data, actions, batch_size):
		...
		for key in minibatch_keys:
			_input,_ouput = self.input_output_sequence(data,actions,key)
			minibatch_input.append(_input)
			minibatch_output.append(_ouput)
		
		return minibatch_input,minibatch_output
	
	def input_output_sequence(self, data, actions, seq_key):
		data_k = data[seq_key]
	...
	def predict(self, data):
		...
		for key in sequence_keys:
			...
			for i in range(0,len(data_k)-2):
				...
				self._predicted[key].append(feature_predicter(input_vect)[0])

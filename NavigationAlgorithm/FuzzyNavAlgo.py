...
class FuzzyNavigationAlgorithm(AbstractNavigationAlgorithm):
	...
	#
	def __init__(self, sensors, target, cmdargs, using_safe_mode=False, with_predictor=False):
		...
		if with_predictor:
			self._obstacle_predictor = HMMObstaclePredictor(360, self._radar.radius);
	...
	def select_next_action(self):
		...
		raw_dynamic_radar_data = self._radar.scan_dynamic_obstacles(self._gps.location());


		if self._with_predictor:
			self._obstacle_predictor.add_observation(self._gps.location(),
					raw_radar_data,
	...
	def _create_obstacle_predictor_pdf(self):
		...
		for angle in np.arange(0, 360, self._PDF.degree_resolution):
			point = self._gps.location() + Vector.unitVectorFromAngle(angle*np.pi/180)*self._normal_speed*2.0;
	...
	def _create_targetpoint_pdf(self):
		targetpoint_pdf = self._PDF.get_distribution(self._gps.angle_to(self._target.position))
		if (self._cmdargs.target_distribution_type == 'dotproduct'):
			targetpoint_ang = self._gps.angle_to(self._target.position)
	...
	def _select_speed(self, direction, raw_dynamic_radar_data):
		...
		if (self.using_safe_mode):
			dynamic_pdf = raw_dynamic_radar_data / self._radar.radius;
	...
	def _calc_memory_bias_vector(self):
		...
		for point in self.visited_points[-size:]:
			effect_magnitude = gaussian_derivative(Vector.getDistanceBetweenPoints(point, self._gps.location()))
			effect_vector = (decay**i) * effect_magnitude * np.subtract(point, self._gps.location())
			vec += effect_vector
			i -= 1
		return vec

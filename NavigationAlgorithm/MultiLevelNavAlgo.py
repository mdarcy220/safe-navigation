...
class MultiLevelNavigationAlgorithm(AbstractNavigationAlgorithm):
	...
	def __init__(self, sensors, target, cmdargs):
		...
		self._localplanner = SamplingNavigationAlgorithm(sensors, target, cmdargs);
		...
		self._node_list = [];
		self._node_list.append(Node(self._gps.location()));
		self._goal_node = Node(self._target.position);
		self._node_list.append(self._goal_node);

		self.debug_info['node_list'] = self._node_list;

		self._radar_data = np.zeros(self._radar.get_data_size());
		self._cur_path = [];
	...
	def _select_next_action_local(self):
		...
		angle = Vector.getAngleBetweenPoints(self._gps.location(), next_waypoint.pos);
		speed = Vector.getDistanceBetweenPoints(self._gps.location(), next_waypoint.pos);
	...
	def _is_at_node(self, node):
		return Vector.getDistanceBetweenPoints(self._gps.location(), node.pos) <= self._POINT_RADIUS;
	...
	def _get_nodes_to_be_checked(self, cur_node):
		...
		if len(check_points) < self._graph_branch_factor:
			...
			for i in range(0, int((self._graph_branch_factor - len(check_points)))):
				...
				max_dist = min(self._CHECK_RADIUS / 1.01, self._radar_data[int(np.round(angle/self._radar.get_degree_step()))]);
				dist = self._radar_data[int(np.round(angle/self._radar.get_degree_step()))]*(np.random.uniform(low=0.6, high=0.89));
	...
	def _has_local_path_to(self, position):
		ang = Vector.getAngleBetweenPoints(self._gps.location(), position);
		index = int(np.round(ang)) % 360;

		return Vector.getDistanceBetweenPoints(self._gps.location(), position) < self._radar_data[index]-5;

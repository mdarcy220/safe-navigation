...
class MpRrtNavigationAlgorithm(AbstractNavigationAlgorithm):
	...
	def _chose_target(self, qgoal):
		...
		if randReal < self._goalBias:
			return Node((self._target[0], self._target[1], np.random.uniform(low=self._time, high=self._time + self._minTimeMultiplier * self._minTime(self._robot.location, self._target))))#qgoal;
		else:
